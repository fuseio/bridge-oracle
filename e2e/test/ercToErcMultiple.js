...
const abisDir = path.join(__dirname, '..', 'submodules/bridge-contracts/build/contracts')
...
const HOME_TOKEN_ADDRESS_1 = '0xA349DCbE3DeF728275535814DC8bFB85707a8dFa'
const FOREIGN_TOKEN_ADDRESS_1 = '0xc26Aa60Ff574f157616D3aEE70e08aAC129E1dFC'
...
const HOME_TOKEN_ADDRESS_2 = '0x7d7678252b927862C8b604ed06f89E76fc9d08c9'
const FOREIGN_TOKEN_ADDRESS_2 = '0xc2176B94D8a62c454e8490cdAd9FFDa5094c32F3'

// multiple bridge with foreign token same as FOREIGN_TOKEN_ADDRESS_1
const HOME_BRIDGE_ADDRESS_3 = '0xad3bF610339C8D61ed6Ef8CE84Bf077A013852a6'
const FOREIGN_BRIDGE_ADDRESS_3 = '0x7d7678252b927862C8b604ed06f89E76fc9d08c9'
const HOME_TOKEN_ADDRESS_3 = '0x846D66d4D246e239555f17150B63C1a2Ef4327F1'
const FOREIGN_TOKEN_ADDRESS_3 = '0xc26Aa60Ff574f157616D3aEE70e08aAC129E1dFC'
...
const erc677Token1 = new homeWeb3.eth.Contract(
  tokenAbi,
  HOME_TOKEN_ADDRESS_1
)
const erc20Token1 = new foreignWeb3.eth.Contract(
  tokenAbi,
  FOREIGN_TOKEN_ADDRESS_1
)

const erc677Token2 = new homeWeb3.eth.Contract(
  tokenAbi,
  HOME_TOKEN_ADDRESS_2
)
const erc20Token2 = new foreignWeb3.eth.Contract(
  tokenAbi,
  FOREIGN_TOKEN_ADDRESS_2
)

const erc677Token3 = new homeWeb3.eth.Contract(
  tokenAbi,
  HOME_TOKEN_ADDRESS_3
)
const erc20Token3 = new foreignWeb3.eth.Contract(
  tokenAbi,
  FOREIGN_TOKEN_ADDRESS_3
)
...
describe('erc to erc (multiple)', () => {
  ...
  describe('3nd deployed bridge', () => {
    it('should convert tokens in foreign to tokens in home', async () => {
      const foreignBalance = await erc20Token3.methods.balanceOf(user.address).call()
      assert(!toBN(foreignBalance).isZero(), 'Account should have tokens in foreign')

      const homeBalance = await erc677Token3.methods.balanceOf(user.address).call()
      assert(toBN(homeBalance).isZero(), 'Account should not have tokens in home')

      const amount = homeWeb3.utils.toWei('0.01')

      // send tokens to foreign bridge
      await erc20Token3.methods
        .transfer(FOREIGN_BRIDGE_ADDRESS_3, amount)
        .send({
          from: user.address,
          gas: '1000000'
        })
      ...
      // check that balance increases
      await promiseRetry(async retry => {
        const balance = await erc677Token3.methods.balanceOf(user.address).call()
        if (!toBN(balance).eq(toBN(amount))) {
          retry()
        }
      })
    ...
    it('should convert tokens in home to tokens in foreign', async () => {
      ...
      // send transaction to home bridge
      const depositTx = await erc677Token3.methods
        .transferAndCall(HOME_BRIDGE_ADDRESS_3, amount, '0x')
        .send({
          from: user.address,
          gas: '1000000'
        })
      ...
      // check that balance increases
      await promiseRetry(async retry => {
        const balance = await erc20Token3.methods.balanceOf(user.address).call()
        const shouldBeBalance = toBN(foreignBalance).add(toBN(amount))
        if (!toBN(balance).eq(shouldBeBalance)) {
          retry()
        }
